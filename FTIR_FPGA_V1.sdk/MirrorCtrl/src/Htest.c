...
{
	usleep(1000000);
	DAC7631_mWriteReg(XPAR_DAC7631_0_S00_AXI_BASEADDR,DAC7631_S00_AXI_SLV_REG3_OFFSET,(u32)0<<31);
	Hdac7631_init();
	u32 value = 65535/2;
	debug_io_test();
	int cnt=0;
	...
	{
		cnt++;
		//Hdac7631_SetCoil(HDAC_5394_COIL_B , value);
		//usleep(10000);
		Hdac7631_SetCoil(HDAC_5394_COIL_A , value);
		value+=10;
		//Hdac7631_SetCoil(HDAC_5394_COIL_A , value);
		usleep(1000000);
		if(cnt>=1)
...
void LaHandler(void *CallbackRef)
{
	u32 dst = 0xf8f01284;
...
void la_int()
{
	axi_pwm_init(150000000u,150000000-25);
	//ak5394_int_v1();

	XScuGic_Connect(&LinIntcInstance, 62,(Xil_InterruptHandler)LaHandler, NULL);
	XScuGic_Enable(&LinIntcInstance,62);
	XScuGic_SetPriorityTriggerType(&LinIntcInstance,62,0, 0);//
	while(1)
	{
		usleep(1000);
	}
...
void IRQ62_TEST(void *CallbackRef)
{
	u32 it_flag = Xil_In32(0xf8f01284);
	it_flag |= (1u<<30);
	Xil_Out32(0xf8f01284,it_flag);

	printf("CPU2进入62号激光中断 \r\n");
...
//					&LinIntcInstance);
	printf("CPU2开始\r\n");
		...
		AXI_SIG_CFG_mWriteReg(XPAR_AXI_SIG_CFG_0_S00_AXI_BASEADDR,
									AXI_SIG_CFG_S00_AXI_SLV_REG1_OFFSET, 50U);
		XScuGic_InterruptMaptoCpu(&LinIntcInstance, 2, 62);
		XScuGic_Connect(&LinIntcInstance, 62,(Xil_InterruptHandler)IRQ62_TEST, NULL);
